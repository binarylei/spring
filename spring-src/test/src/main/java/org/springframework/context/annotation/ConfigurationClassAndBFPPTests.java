...
public class ConfigurationClassAndBFPPTests {
    ...
    // 非静态方法定义 BeanFactoryPostProcessor 时影响其配置类的生命周期
    // 因为 @Bean public BeanFactoryPostProcessor bfpp() 所以必须在 invokeBeanFactoryPostProcessors
    // 阶段就实例这个 BeanFactoryPostProcessor，而该方法又是非静态方法，所以会实例化配置类 AutowiredConfigWithBFPPAsInstanceMethod
    // 但如果是静态方法就不一样了，不会实例化该配置类
    @Test
    ...
    static class AutowiredConfigWithBFPPAsInstanceMethod {
        ...
        public AutowiredConfigWithBFPPAsInstanceMethod() {
            System.out.println("instant non-static");
        }
    ...
    static class AutowiredConfigWithBFPPAsStaticMethod {
        ...
        public AutowiredConfigWithBFPPAsStaticMethod() {
            System.out.println("instant static");
        }
