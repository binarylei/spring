...
public class AnnotatedElementUtilsTests {
    ...
    // declaredAnnotations 元注解 + @Inherited 注解
    @Test
    ...
    /**
     * 在 declaredAnnotations 中已经找到后就不会在 inheritedAnnotations 中继续查找了
    ...
    public void getMergedAnnotationWithAliasedValueComposedAnnotation() {
        Set<ContextConfig> allMergedAnnotations = getAllMergedAnnotations(AliasedValueComposedContextConfigClass.class, ContextConfig.class);

        assertGetMergedAnnotation(AliasedValueComposedContextConfigClass.class, "test.xml");
    }

    // value groovyScripts xmlFiles() locations() 均指向同一个属性
    @Test
    ...
    // 指定的元注解层级不一致 ContextConfig  ImplicitAliasesContextConfig
    @Test
